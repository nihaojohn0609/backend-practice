...
### 4. 댓글 관리
* 게시글별 댓글 생성, 조회
* 댓글 수정 및 삭제 (작성자만 가능하도록 권한 검증)
* 회원(Member), 게시글(Post), 댓글(Comment) 간의 연관관계 매핑

### 5. Frontend (React)
* 로그인 / 회원가입 페이지
* 게시글 목록, 상세 조회, 작성, 수정, 삭제
* 게시글 상세 페이지 내 댓글 작성, 수정, 삭제
...
| POST | `/posts/{id}/comments` | 댓글 생성 | O |
| GET | `/posts/{id}/comments` | 게시글별 댓글 조회 | O |
| PUT | `/posts/{id}/comments/{commentId}` | 댓글 수정 | O |
| DELETE | `/posts/{id}/comments/{commentId}` | 댓글 삭제 | O |
...
./mvnw test -Dtest=CommentServiceImplTest  # 댓글 서비스 테스트
```